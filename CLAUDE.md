...
**Generic benchmark model:** a holding has N benchmarks, each an ETF tagged with `benchmark_type` (`sector`, `cap_style`, `market`, `bond_market`, `duration`). The comparison is always `holding% − benchmark%`, computed per benchmark. Adding an axis later is a seed row, not a rewrite. The mapping lives in the `benchmark_etfs` seed (`transformation/seeds/benchmark_etfs.csv`): `benchmark_type, lookup_key, etf_ticker, etf_name`.
