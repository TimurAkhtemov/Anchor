...
1. **Unattended post-close operation.** Dagster+ Serverless configuration, build-time
   manifest packaging, public-only secrets, weekday scheduling, snapshot validation,
   and atomic publication are implemented. External activation remains an explicitly
   authorized operator step. The graph is demo-only; real/SnapTrade pulls stay private.
